...
kable(data2, digits = 2, caption = "Medias de las variables físico-químicas medidas en las muestras de los suelos de Puchuncaví. Cabe mencionar que EC corresponde a la electroconductividad, OM se refiere al porcentaje de materia orgánica en los suelos analizados, sand corresponde al porcentaje de arena, clay al porcentaje de arcilla y silt es el porcentaje de limo. En cuanto a los metales pesados estos son presentados en unidades de mg/Kg") %>% kable_styling(latex_options = "striped")
```
...
kable(data4, digits = 2, caption = "Medias de las variables biológicas medidas en las muestras de los suelos de Puchuncaví. Cabe mencionar que la columna bacterias corresponde al recuento bacteriano presente en los suelos expresado en UFC/gr de suelo") %>% kable_styling(latex_options = "striped")
...
fit2 <- glance(fit1)
kable(fit2, digits = 2, caption = "Indicadores estadísticos de modelo propuesto diversidad explicado por la variable pH") %>% kable_styling(latex_options = "striped")
```

```{r data5, echo=FALSE,message=FALSE}
fit3 <-tidy(fit1)
kable(fit3, digits = 2, caption = "Indicadores estadísticos de modelo propuesto donde la diversidad es explicada a partir de la variable pH obtenidos mediante tidy.") %>% kable_styling(latex_options = "striped")
```

```{r data6, echo=FALSE,message=FALSE}
fit4 <- augment(fit1)
kable(fit4, digits = 2, caption = "Predicciones y residuales del modelo propuesto en el estudio: diveridad explicada por la variable pH.") %>% kable_styling(latex_options = "striped")
```
...
Modelos <- bind_rows(Modelo1, Modelo2, Modelo3, Modelo4, Modelo5) %>% arrange(AIC) %>% mutate(DeltaAIC = AIC - 
    min(AIC))
kable(Modelos, digits = 2, caption = "Tabla con valores de $r^2$ y valor del criterio de información de Akaike (AIC) de los diferentes modelos propuestos en el estudio") %>% kable_styling(latex_options = "striped")
```
